...
let Enventory = SpriteKind.create()
...
let pistol_shape = 0
let enventory = [["test", "", "", "", "", "", "", ""], ["", "", "", "", "", "", "", ""], ["", "", "", "", "", "", "", ""]]
let enventory_sprite = sprites.create(img`.`, Enventory)
let enventoryx = 0
let enventoryy = 0
let enventorying = false
let enventory_items = {
    "test" : sprites.create(img`.`, Enventory),
...
function update_enventory() {
    if (enventorying) {
        enventory_sprite.setImage(img`
            ...
            ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
        `)
        enventory_sprite.setPosition(user.x, user.y)
        /** 
        for i in range(3):
            for j in range(9):
...
controller.A.onEvent(ControllerButtonEvent.Pressed, function A_pressed() {
    ...
    if (weapon == 1) {
        ...
        enventorying = !enventorying
    }
    
})
game.onUpdate(function on_on_update() {
    if (enventorying) {
        controller.moveSprite(user, 0, 0)
    } else if (!enventorying) {
        controller.moveSprite(user, 100, 100)
    }
